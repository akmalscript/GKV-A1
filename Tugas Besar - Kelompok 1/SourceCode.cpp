/**
 * <<< CATATAN >>>
 * Code ini dibuat & di-run menggunakan VS Code
 * Silahkan gunakan tutorial berikut untuk setup VS Code
 * https://sugar-script-430.notion.site/Tutorial-Setup-GLUT-di-VSCode-1b1bded9d4fb80249cb1d771dc524f8b?pvs=4
 * 
 * Terima Kasih
 */
...
GLfloat ambient[] = { 0.1f, 0.1f, 0.15f, 1.0f };     // sangat redup, biru lembut
...
void buatLangitMalam() {
    // ===== Inisialisasi STAR POSITIONS SEKALI SAJA =====
    ...
    glPopMatrix();
      ...
      // 3) Bintang (pakai posisi yang sudah di-generate sekali tadi)
      glPointSize(2.0f);
      glBegin(GL_POINTS);
...
void buatLangitSiang() {
    ...
    // 3) Awan-awan
    // -- Pastikan digambar di sini, saat lighting dimatikan
...
bool checkCollision() {
    ...
    for (const auto& obstacle : obstacles) {
        ...
        float collisionThreshold = (obstacle.type == 0) ? 1.5f : 1.8f; // Batu atau pohon
...
void generateDecorations() {
    ...
    // Gunakan seed yang sama dengan obstacle agar tetap konsisten
...
void display() {
    ...
    for (const auto& obstacle : obstacles) {
        if (obstacle.z > carZ - 10 && obstacle.z < carZ + 100) { // hanya gambar yang di depan mobil
            if (obstacle.type == 0) {
    ...
    if (gameOver) {
        ...
        glColor3f(1.0, 1.0, 1.0);
        renderSmallText(300, 280, "Tekan R untuk restart");
    } else {    
        if (carZ >= finishZ) {
...
int main(int argc, char** argv) {
    ...
    glShadeModel(GL_SMOOTH); // Untuk shading yang lebih halus
